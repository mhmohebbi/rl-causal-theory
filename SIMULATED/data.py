...
class AbstractDataset(Dataset):
    ...
    def download_csv(self):
        self.df.to_csv(f"./SIMULATED/datasets/{self.name}.csv", index=False)
    ...
    def check_casual_graph(self, baseline_model_name, timestamp):
        ...
        graph_viz.render(f'./SIMULATED/results/{timestamp}/causal_graphs/{baseline_model_name}/{self.name}', format='png', cleanup=True)
    ...
    def find_independent_features(self, df, baseline_model_name, timestamp, corr_threshold=0.5):
        ...
        os.makedirs(f'./SIMULATED/results/{timestamp}/correlations/{baseline_model_name}', exist_ok=True)
        plt.savefig(f'./SIMULATED/results/{timestamp}/correlations/{baseline_model_name}/correlation_matrix_{self.name}.png')
